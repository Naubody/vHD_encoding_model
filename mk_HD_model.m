...
angles = arrayfun(@(x) deg2rad(0:kernels(x):359), 1:numel(kernels), 'uni', 0);
kappa = arrayfun(@(x) findKappa(kernels(x)), 1:numel(kernels), 'uni', 1); % find kappa with optimal overlap
model = arrayfun(@(y) cell2mat(arrayfun(@(x) circ_vmpdf(deg2rad(0:359), angles{y}(x), kappa(y)), 1:length(angles{y}), 'uni', 0))', 1:numel(kernels), 'uni', 0);
end
